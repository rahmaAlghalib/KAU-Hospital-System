...
public class KAUHospitalSystem {
   ...
   //GET DOCTOR METHOD------------------------------
    private static Doctor getDoctor(Scanner input) {
        ...
        Doctor doctor = new Doctor(consultationFees, specialization, staffid, id, name, nationality, gender, phone);
   ...
   //GET NURSE METHOD-----------------------------
    private static Nurse getNurse(Scanner input) {
        ...
        Nurse nurse = new Nurse(experience, specialization, staffid, id, name, nationality, gender, phone);
        ...
        do {
           ...
           // print details
            ...
            for (int i = 0; i < patientsArraySize; i++) {  
                ...
                output.println("\tDoctor : " + patients.get(i).getDoctor().getName() + "   Doctor Consultation Fees: " + patients.get(i).getDoctor().getConsultationFees() + " SR" );
                output.println("\tMedicines  are:");
                
                for (int j = 0; j < patients.get(i).getTotalMedicine(); j++) {
                    output.printf("%s%s%s","\tMedicine name: " , patients.get(i).getMedicine(j).getName() + "   \t" , "Price: " + patients.get(i).getMedicine(j).getPrice() + " SR");
                    output.println();
                    
                }
                output.println("\tRoom: " + patients.get(i).getRoom().getRoomNo() + " Room Charge: " + patients.get(i).getRoom().getCharges() + " SR   ");
                output.println("\t\t==== Total Charges :" + patients.get(i).calTotalBill() + " SR ====");
                output.println("----------------------------------------------------------------");
